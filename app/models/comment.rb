...
class Comment < ApplicationRecord
  ...
  # Stop accepting replies this deep. Recursive CTE requires a fixed max ,
  ...
  validates :confidence, :flags, :score, presence: true
  ...
  def self.recent_threads(user)
    ...
    Comment
      .joins(<<~SQL
        inner join (
          with recursive discussion(id, depth, reply_count, path) as (
          select
            c.id,
            0 as depth,
            (select count(*) from comments where parent_comment_id = c.id) as reply_count,
            ARRAY[ -confidence, c.id]::DECIMAL[]
            from comments c
            where
      ...
      .order("comments.thread_id desc, comments_recursive.path")
      .select('
        comments.*,
  ...
  def self.story_threads(story)
    ...
    Comment
      ...
      .order("comments_recursive.path")
      .select('
        comments.*,
        comments_recursive.depth as depth,
        comments_recursive.reply_count as reply_count,
        comments_recursive.path as path
      ')
  end
end
